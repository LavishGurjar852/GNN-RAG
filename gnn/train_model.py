...
class Trainer_KBQA(object):
    ...
    def evaluate_best(self):
        ...
        # self.logger.info("TEST F1: {:.4f}, H1: {:.4f}, EM {:.4f}".format(eval_f1, eval_h1, eval_em))
        checkpoints = ["h1", "f1", "final"]
        for ckpt in checkpoints:
            filename = os.path.join(self.args['checkpoint_dir'], f"{self.args['experiment_name']}-{ckpt}.ckpt")
            if os.path.exists(filename):
                self.logger.info(f"Loading checkpoint from {filename}")
                self.load_ckpt(filename)
                eval_f1, eval_h1, eval_em = self.evaluate(self.test_data, self.test_batch_size, write_info=False)
                self.logger.info(f"Best {ckpt} evaluation")
                self.logger.info(f"TEST F1: {eval_f1:.4f}, H1: {eval_h1:.4f}, EM {eval_em:.4f}")
            else:
                self.logger.warning(f"Checkpoint file {filename} does not exist.")
